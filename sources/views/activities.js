	...
	config() {
		...
		var activitiesDatatable = {
			...
			columns: [
				...
				{ id: "TypeID", header: [ _("Activity type"), { content: "selectFilter" }], sort: "string", options: /*{ body: { data: activityTypes, template: "#Value#" } }*/activityTypes, template: "#Value#", width: 250 },
		...
		this.$$("activitiesDatatable").registerFilter(
			this.$$("activitiesToolbar"),
			{
				columnId: "DueDate", compare: (value, filter, item) => {
					...
					switch (filter) {
						case "overdue":
							return today - value > 0 && item.State == "Open";
							break;
						case "completed":
